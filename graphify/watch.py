...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False) -> bool:
    ...
    try:
        ...
        # Preserve the semantic (non-code) layer. A code-only rebuild (git hook or
        # --watch) must never clobber doc/concept/decision/normativa nodes produced
        # by LLM extraction — only 'code' file_type nodes are regenerated here.
        out = watch_path / "graphify-out"
        existing_path = out / "graph.json"
        preserved_sem_nodes = 0
        if existing_path.exists():
            try:
                existing = json.loads(existing_path.read_text(encoding="utf-8"))
                ex_nodes = existing.get("nodes", [])
                ex_links = existing.get("links", existing.get("edges", []))
                fresh_code_ids = {n["id"] for n in result["nodes"]}
                sem_nodes = [n for n in ex_nodes
                             if n.get("file_type") != "code" and n["id"] not in fresh_code_ids]
                sem_ids = {n["id"] for n in sem_nodes}
                live_ids = fresh_code_ids | sem_ids
                # keep edges touching a preserved semantic node whose endpoints
                # both still exist; stale code<->code edges are regenerated by AST
                sem_edges = [e for e in ex_links
                             if (e.get("source") in sem_ids or e.get("target") in sem_ids)
                             and e.get("source") in live_ids and e.get("target") in live_ids]
                result = {
                ...
                preserved_sem_nodes = len(sem_nodes)
            except Exception as exc:
                print(f"[graphify] semantic layer not merged (code-only rebuild): {exc}")
